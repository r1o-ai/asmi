...
/// Try to read a named tensor, returning `None` if it does not exist.
pub fn try_tensor_to_f32(safetensors: &SafeTensors, name: &str) -> Option<Box<[f32]>> {
    safetensors.tensor(name).ok().map(|tensor| {
        let bytes = tensor.data();
        match tensor.dtype() {
            Dtype::BF16 => bytes
                .chunks_exact(2)
...
///
/// NOTE: Qwen 3.5 uses standard PyTorch [out_features, in_features] layout,
/// so this function is generally NOT needed for Qwen weights. Kept for
/// compatibility with any future weight formats that may require transposing.
