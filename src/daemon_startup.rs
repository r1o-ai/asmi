...
pub async fn run_serve(port: u16, bind: String, interval: u64, cluster_hub: bool, cli_models_dir: Vec<String>, _experimental_ane: bool, experimental_egpu: bool) -> Result<()> {
    ...
    // Background polling loop — collect local metrics every N seconds
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                // Auto-recover Error-state managers + adopt unmanaged servers on idle ports
                for mgr in serve_managers.read().await.values() {
                    mgr.check_port_recovery().await;
    ...
    // RDMA auto-setup runs in background so it doesn't block HTTP bind
    // when peers are unreachable (SSH timeouts would stall the health endpoint)
    {
    ...
    let app_state = daemon::AppState {
        ...
        egpu_cache,
        jaccl_groups: std::sync::Arc::new(std::sync::Mutex::new(std::collections::HashMap::new())),
    };
