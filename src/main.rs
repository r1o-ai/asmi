...
struct Cli {
    ...
    /// Bind address for --serve mode. Default 0.0.0.0 so peer daemons in the
    /// cluster mesh can reach this node over TB5/RDMA or LAN. Set to 127.0.0.1
    /// when running `tailscale serve` on the same port (it owns the Tailscale
    /// interface and forwards to localhost).
...
async fn main() -> Result<()> {
    ...
    if args.serve {
        return daemon_startup::run_serve(
            args.port, args.bind, args.interval, args.cluster,
